...
def home(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save(user=request.user)  # Pass the currently logged-in user to the form

            return HttpResponseRedirect('#')
    else:
        form = AddDateForm(initial={'user': request.user})  # Set the initial value for the 'user' field
    ...
    for all in total:
        for item in items:
            if item.date_d is not None and all.date.id == item.date_d.id:
                date = item.date_d.id
                # Check if the date is already in date_list
                if not any(entry['date'] == date for entry in date_list):
                    # If not, add it to date_list
                    date_list.append({
                        'date': date,
                        'protien': item.add_item.Protein,
                        'carbo': item.add_item.Carbohydrates,
                        'fat': item.add_item.Fat,
                        'calories': item.add_item.Calories,
                    })
...
#         if form.is_valid():
#             form.user = request.user  # Pass the currently logged-in user to the form
#             form.save()
